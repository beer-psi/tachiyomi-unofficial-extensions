...
data class PageDto(
    ...
    fun toPage(): Page {
        ...
        if (dto.status == "enqueued") {
            throw Exception("Đang đợi xử lý hình ảnh, vui lòng chờ ít phút.")
        }

        if (dto.status == "processing") {
            throw Exception("Đang xử lý hình ảnh, vui lòng chờ ít phút.")
        }

        if (dto.status == "failed") {
            throw Exception("Xử lý hình ảnh thất bại.")
        }
